...
Misakas at the bottom act as widgets to let users customize the display and navigate the data. The two Misakas at the bottom specify the time range, they act like a range slider\cite{ahlberg1992dynamic}(see Figure ). For instance, moving the right Misaka to the right, the line chart will show a wider time range.

% 画一下Zooids Figure 4. A line chart visualization using zooids.

\subsection{Multiple scatterplots}

Scatterplots is a type of plot or mathematical diagram using Cartesian coordinates to display values for typically two variables for a set of data\cite{Scatterplot}. 

With a group of Misakas, each with RGB LED on the top, 
